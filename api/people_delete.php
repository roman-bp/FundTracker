...
// перевірка існування
$chk = $conn->prepare("SELECT id, full_name FROM people WHERE id = ? LIMIT 1");
...
$stmt = $conn->prepare("DELETE FROM people WHERE id = ? LIMIT 1");
if (!$stmt) out(["error"=>"Prepare failed"], 500);
$stmt->bind_param("i", $id);

if (!$stmt->execute()) out(["error"=>"Delete failed"], 500);

out(["ok"=>true, "deleted_id"=>$id, "full_name"=>$row["full_name"]]);
